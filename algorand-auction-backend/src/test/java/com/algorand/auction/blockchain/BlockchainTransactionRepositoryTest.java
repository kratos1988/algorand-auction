...
class BlockchainTransactionRepositoryTest {
    ...
    void saveTransaction() throws Exception {
        ...
        BlockchainTransactionRepository underTest = new BlockchainTransactionRepository(
                balanceChecker,
                transactionSender);
        underTest.saveTransaction(senderPublicKey, receiverPublicKey, amount, "do li sordi per un oggetto");
        verify(transactionSender).send(senderAddress, receiverAddress, amount, "do li sordi per un oggetto");
    }
    ...
    void notSaveTransactionWhenBalanceIsNotEnough() throws Exception {
        ...
        BlockchainTransactionRepository underTest = new BlockchainTransactionRepository(
                ...
                transactionSender);
        underTest.saveTransaction(senderPublicKey, receiverPublicKey, amount, "do li sordi per un oggetto");
        verify(transactionSender, never()).send(senderAddress, receiverAddress, amount, "do li sordi per un oggetto");
    }
